...
module.exports = {  
        ...
        caozuo:"/game/g_game.ashx?action=caozuo&grade={grade}&uid={uid}&no={no}&actype={actype}",//游戏操作(1 种树  2浇水，3施肥   4采摘 11一键施肥可以不传No参数) (grade 等级，no 编号,actype 操作类型)-
        ...
        urlreplace:function(url,lst) { 
            ...
            //  var key=mymd5.str_md5(this.onekey);
            //      key=mymd5.str_md5(key.substr(2,8)) ;
                     ...
                     if (xhr.readyState == 4) {
                         if(xhr.status >= 200 && xhr.status < 400){
                            ...
                            if(response.msg2 =='非法操作'){
                                ...
                                var node_ = new cc.Node();
                                node_.color = new cc.Color(255, 0, 0);
                                var label_ = new cc.Label();
                                label_.string = response.msg3;
                                //node_.addComponent(label_);
                                node_.addComponent(cc.Label);
                                 node_.addComponent(cc.Button);
                                node_.setPosition(600,400);
                               
                                node_.getComponent(cc.Label).string = '版本已更新';

                              
                                node_.getComponent(cc.Button).node.on('click',function(){
                                    console.log('回调!!!!!!!!!!!!!!');
                                    window.location.href = response.msg3;
                                });
                               
                                cc.log(label_.string);
                            ...
                            var response = {'msg1':'error','msg2':'网络请求超时','msg3':''};
                            console.log('网络联机失败:'+response)
                            fun(response)
                        }
        ...
        {     console.log("-----------------------a-----------------------");
         ...
         treeAction: function(list,fun){
             ...
             console.log('no:'+list.no);
             console.log('actype:'+list.actype);
